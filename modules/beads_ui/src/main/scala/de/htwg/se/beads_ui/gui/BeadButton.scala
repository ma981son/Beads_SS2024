...
import de.htwg.se.beads_util.Enums.{SelectedColor, ColorConverter}
...
class BeadButton(row: Int, col: Int, controller: ControllerInterface)
  ...
  style =
    s"-fx-background-color: ${ColorConverter.colorToRGBA(bead.beadColor)}; ${BeadButton.defaultStyle}"
